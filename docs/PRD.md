...
> **版本**: 26.7.31+0505 · **文档类型**: Product Requirements Document
>
> 📎 [配套技术文档](./TS.md)
...
| ID | 功能 | 描述 |
|----|------|------|
| F-09 | **用量统计** | 按日聚合的 token 用量和请求数图表 |
| F-10 | **模型注册** | 管理可用模型列表，支持层级分类 |
| F-11 | **Provider 启用/禁用** | 快速切换 Provider 是否参与路由 |
| F-12 | **健康检查** | `/health` 端点，供监控系统探活 |

#### P2 — 锦上添花（可以有）

| ID | 功能 | 描述 |
|----|------|------|
| F-13 | **路由规则** | 基于优先级和权重的多 Provider 负载分发 |
| F-14 | **成本估算** | 基于模型定价估算每次请求的费用 |
| F-15 | **暗色模式** | 跟随系统自动切换明/暗主题 |
| F-16 | **WebSocket 实时推送** | 管理面板实时获取状态更新 |
| F-17 | **WebSearch 劫持** | 可选拦截含 web_search tool 的请求，用本地 Bing 搜索代替上游 Provider 执行搜索 |
| F-18 | **模型同步** | 从上游 Provider 自动拉取可用模型列表 |
| F-19 | **Dashboard API** | 概览和用量统计端点 |
| F-20 | **系统托盘** | 关闭窗口时最小化到托盘继续运行 |
...
> **V5 迁移后**：密钥健康状态改为纯内存管理，启动时所有 Key 均为绿色初始状态。DB 中的 `status` 列不再被读写，状态不再从数据库恢复 🐾
