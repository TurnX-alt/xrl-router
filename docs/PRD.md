...
| US-05 | 开发者 | 作为开发者，我想查看按日/小时统计的 token 用量图表和数据磁贴，以便了解消耗趋势、缓存命中率和最爱用的模型 | StatsView 页面展示数据磁贴 + 折线图/柱状图，默认「当天」时间范围 |
...
| ID | 功能 | 描述 |
|----|------|------|
| F-11 | **用量统计** | 按日/小时聚合的 token 用量和请求数图表 + 数据磁贴（总消耗、使用最多模型、请求次、输入/输出/命中、缓存命中率） |
